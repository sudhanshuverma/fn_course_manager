...
global $CFG, $OUTPUT, $FULLME,$PAGE, $COURSE, $DB, $FULLME, $PAGE;
...
if(($patharray['filename']==='modedit') && empty($type) && empty($add) && !empty($update) && $haseditingcapability ){
    ...
    $previewlink = "{$CFG->wwwroot}/mod/{$modname}/view.php?id=$modid";
    $editcontentlink = "{$CFG->wwwroot}/blocks/course_menu/edit.php?update={$modid}&return=0";
    $settinglink = "{$CFG->wwwroot}/course/modedit.php?update={$modid}&return=0";
