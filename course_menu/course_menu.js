...
M.block_course_menu = M.block_course_menu || {
    ...
    init:function(Y) {
     ...
     create_tab:function(Y, modid, filename, modnamearr, basename, modname, modnameexistinmodnamearr,selectedtab) {       
        var mainDiv = document.getElementById("region-main");   
      
        
            if (  ((filename === 'modedit') && (basename === 'course')) || 
                 ((filename === 'view') && (modnameexistinmodnamearr===true)) ){
                ...
                for(var i=1; i<4; i++) {
                    ...
                    if (i === 1) {
                        var viewPageUrl = M.cfg.wwwroot+'/mod/'+modname+'/view.php?id='+modid; 
                        ...
                        var editContentPageUrl = "";//M.cfg.wwwroot+'/mod/'+modname+'/view.php?id='+modid; 
                        ulist.appendChild(liList);
                        liList.appendChild(liLink);
                        ...
                        var settingPageUrl = M.cfg.wwwroot+'/course/modedit.php?update='+modid+'&return=0'; 
                        ulist.appendChild(liList);
                        liList.appendChild(liLink);
